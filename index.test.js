...
describe('Movie Database', () => {
    ...
    test('Movies can have many cast', async() => {
        const testMovie =  await Movie.findOne({where: {title: 'Zootopia'}})
        const testCast1 = await Cast.findOne({where: {cast_name: 'Crystal Johnson'}})
        const testCast2 = await Cast.findOne({where: {cast_name: 'Ginnifer Goodwin'}})
        ...
        expect(castList[1].cast_name).toMatch('Ginnifer Goodwin')
    })
    test('Movies can have many crew', async() => {
        const testMovie2 = await Movie.findOne({where: {title: 'Malcolm X'}})
        const testCrew1 = await Crew.findOne({where: {name: 'Spike Lee'}})
        const testCrew2 = await Crew.findOne({where: {name: 'Crystal Payne'}})
        await testMovie2.addCrew(testCrew1)
        await testMovie2.addCrew(testCrew2)
        const crewList = await testMovie2.getCrews()
        expect(crewList.length).toBe(2)
        expect(crewList[0] instanceof Crew).toBeTruthy()
        expect(crewList[1].name).toMatch('Crystal Payne')
    })
